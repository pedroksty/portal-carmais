...
export default function IndexFilters({ brands, kinds }) {
  ...
  return (
    <div className='index-filters-container' >
			<h2>Pesquise por marca, categoria ou faixa de preço</h2>
			...
			<div className='index-filters-select-button-container'>
				...
				}
				{
					kinds.length 
					? (<button 
						className={`index-filters-select-button ${selectedSearch == 'categories' ? 'index-filters-select-button-selected' : ''}`}
						onClick={() => setSelectedSearch('categories')} 
					>
						Categorias
					</button>)
					: null
			...
			<div className='filters-container' >
				{
					...
					?	(
						...
						{
							fixedBrands.filter((x, idx) => idx < 9 || windowWidth > 1023).map((brand) => (
