...
export default function GeneralFilters({ filters, units }) {
	...
	const [filtersOpen, setFiltersOpen] = useState(filterStore.sideBarOpen);
	...
	useEffect(() => {
		...
		const minKmParam = filterStore.minKm ? `minKm=${filterStore.minKm}&` : '';
		const maxKmParam = filterStore.maxKm ? `maxKm=${filterStore.maxKm}&` : '';

		router.replace(`${pathName}?${textSearchParam}${minYearParam}${maxYearParam}${minPriceParam}${maxPriceParam}${brandsParam}${unitsParam}${modelsParam}${statesParam}${citiesParam}${stateStringsParam}${shiftsParam}${colorsParam}${selectedPriceRangeParam}${minKmParam}${maxKmParam}`)
	...
	useEffect(() => {
		...
		const timeOutId2 = setTimeout(() => {
			console.log('entrou no timetou')

			useStore.setState({...filterStore, sideBarOpen: false});
		}, 1500);
	...
	useEffect(() => {
		console.log('filterStore.sideBarOpen', filterStore.sideBarOpen)
	}, [filterStore])
	...
	useEffect(() => {
		const timeOutId = setTimeout(() => {
			useStore.setState({ ...filterStore, 'textSearch': awaitSearchInput, sideBarOpen: false })
		}, 1500);
	...
	const handleSelectPriceRange = (option) => {
		if (option === filterStore.selectedPriceRange) {
			useStore.setState({ ...filterStore, minPrice: '', maxPrice: '', selectedPriceRange: 0, sideBarOpen: false })
			return;
		}
		if (option > 2) {
			useStore.setState({ ...filterStore, minPrice: '100000', maxPrice: '', selectedPriceRange: option, sideBarOpen: false })
		} else {
			useStore.setState({ ...filterStore, maxPrice: option === 1 ? '50000' : '100000', minPrice: '', selectedPriceRange: option, sideBarOpen: false })
		}
	}
	...
	useEffect(() => {
		if (awaitMinPriceInput !== '') {
			if (filterStore.selectedPriceRange > 0) {
				useStore.setState({ ...filterStore, selectedPriceRange: 0, minPrice: '', maxPrice: '', sideBarOpen: false })
			}
			const timeOutId2 = setTimeout(() => {
				useStore.setState({ ...filterStore, minPrice: awaitMinPriceInput, sideBarOpen: false })
			}, 1500);
			return () => clearTimeout(timeOutId2)
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	...
	const [awaitMaxKmInput, setAwaitMaxKmInput] = useState('')
	useEffect(() => {
		if (awaitMaxKmInput !== '') {
			const timeOutId = setTimeout(() => {
				console.log('awaitMaxKmInput', awaitMaxKmInput)

				useStore.setState({ ...filterStore, maxKm: awaitMaxKmInput, sideBarOpen: false })
			}, 1500);
			return () => clearTimeout(timeOutId)
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	...
	const [awaitMinKmInput, setAwaitMinKmInput] = useState('')
	useEffect(() => {
		if (awaitMinKmInput !== '') {
			const timeOutId2 = setTimeout(() => {
				useStore.setState({ ...filterStore, minKm: awaitMinKmInput, sideBarOpen: false })
			}, 1500);
			return () => clearTimeout(timeOutId2)
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	...
	useEffect(() => {
		if (awaitMaxPriceInput) {
			...
			const timeOutId = setTimeout(() => {
				useStore.setState({ ...filterStore, maxPrice: awaitMaxPriceInput, sideBarOpen: false })
			}, 1500);
			return () => clearTimeout(timeOutId)
		}
		// eslint-disable-next-line react-hooks/exhaustive-deps
	...
	const handleChangeCheckbox = (e) => {
		if (e.target.checked) {
			useStore.setState({ ...filterStore, [e.target.name]: [...filterStore[e.target.name], e.target.value], sideBarOpen: false })
		} else {
			useStore.setState({
				...filterStore,
				sideBarOpen: false,
	...
	const handleChangeYear = (e, field) => {
		...
		if (year.length == 4) {
			useStore.setState({ ...filterStore, [field]: year, sideBarOpen: false })
		}
	}
				...
				<SideOverlay open={filtersOpen} onClose={closeSideBarHandler} title='Filtros' >
					<div className='side-filters-container' >
						...
						<div className='year-range-inputs-container' >
							<div className='filter-generic-input-container'>
								<div>
									<label htmlFor='minor-price-input' className='filter-default-label' >De</label>
									<input
										type='number'
										className='filter-default-input'
										id='initial-year-input'
										placeholder='de'
										onChange={(e) => setAwaitMinKmInput(e.target.value)}
										value={awaitMinKmInput}
							...
							<div className='filter-generic-input-container filter-generic-input-container-right' >
								<div>
										<label htmlFor='major-price-input' className='filter-default-label' >Para</label>
										<input
											type='number'
											className='filter-default-input'
											id='final-year-input'
											placeholder='para'
											onChange={(e) => setAwaitMaxKmInput(e.target.value)}
											value={awaitMaxKmInput}
									/>
								</div>
							</div>
						</div>
